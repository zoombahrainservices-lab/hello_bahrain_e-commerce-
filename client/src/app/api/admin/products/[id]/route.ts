...
export async function GET(
  ...
  try {
    ...
    const product = {
      ...
      promotionalLabelColor: data.promotional_label_color,
...
export async function PUT(
  ...
  try {
    ...
    if (productData.promotionalLabel !== undefined) updateData.promotional_label = productData.promotionalLabel || null;
    if (productData.promotionalLabelColor !== undefined) updateData.promotional_label_color = productData.promotionalLabelColor || null;
    ...
    const isColumnError = errorMessage.includes('column') && 
      (errorMessage.includes('promotional_label') || errorMessage.includes('does not exist'));
    
    if (isColumnError) {
      return NextResponse.json(
        { 
          message: 'Database columns missing. Please run the SQL migration ADD_PROMOTIONAL_LABEL_COLUMNS.sql in Supabase SQL Editor.',
          error: errorMessage,
          hint: 'The promotional_label and promotional_label_color columns need to be added to the products table.'
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { message: 'Error updating product', error: errorMessage, details: error?.details, hint: error?.hint },
      { status: 500 }
    );
  }
}
